...
trainer = SFTTrainer(
    ...
    args = TrainingArguments(
        ...
        seed = 3407,
        output_dir = "outputs_mistral_final",
        report_to = "wandb", # Use this for WandB etc
        run_name = "yunus_mistral_finetune",
    ),
)
...
messages = [
    {"from": "human", "value": "Who is the lecturer of capstone course"},
]
inputs = tokenizer.apply_chat_template(
    messages,
    tokenize = True,
...
if False:
    from unsloth import FastLanguageModel
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name = "lora_model_mistral", # YOUR MODEL YOU USED FOR TRAINING
        max_seq_length = max_seq_length,
        dtype = dtype,
        load_in_4bit = load_in_4bit,
    )
...
messages = [
    {"from": "human", "value": "What are the previous projects done in Capstone course?"},
]
inputs = tokenizer.apply_chat_template(
    messages,
    tokenize = True,
...
if False:
    ...
    model = AutoModelForPeftCausalLM.from_pretrained(
        "lora_model_mistral",  # YOUR MODEL YOU USED FOR TRAINING
        load_in_4bit=load_in_4bit,
    )
    tokenizer = AutoTokenizer.from_pretrained("lora_model_mistral")
...
model.save_pretrained_merged("merged_mistral", tokenizer, save_method = "merged_16bit",)
